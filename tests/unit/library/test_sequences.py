...
from guidance._grammar import ByteTerminalRule, JoinRule, select
...
class TestExactlynRepeats:
    def test_smoke(self):
        grammar = JoinRule(["AAA", exactly_n_repeats("b", 4), "BBB"])
    ...
    def test_bad_repeats(self, bad_string: str, good_bytes, failure_byte, allowed_bytes):
        PREFIX = "AAA"
        SUFFIX = "BBB"
        grammar = JoinRule([PREFIX, exactly_n_repeats("b", 4), SUFFIX])
        check_match_failure(
...
class TestAtMostnRepeats:
    def test_smoke(self):
        grammar = JoinRule(["AAA", at_most_n_repeats("a", 3), "BBB"])
    ...
    def test_bad_repeats(self, bad_string: str, good_bytes, failure_byte, allowed_bytes):
        PREFIX = "AAA"
        SUFFIX = "BBB"
        grammar = JoinRule([PREFIX, at_most_n_repeats("b", 4), SUFFIX])
        check_match_failure(
            bad_string=PREFIX + bad_string + SUFFIX,
            good_bytes=PREFIX.encode() + good_bytes,
            failure_byte=failure_byte,
...
class TestSequence:
    def test_smoke(self):
        grammar = JoinRule(["AAA", sequence("a"), "BBB"])
    ...
    def test_bad_repeats_unconstrained(
        ...
        PREFIX = "AAA"
        SUFFIX = "BBB"
        grammar = JoinRule([PREFIX, sequence("b"), SUFFIX])
        check_match_failure(
            bad_string=PREFIX + bad_string + SUFFIX,
            good_bytes=PREFIX.encode() + good_bytes,
            failure_byte=failure_byte,
    ...
    def test_bad_repeats_min_length(
        ...
        PREFIX = "AAA"
        SUFFIX = "BBB"
        grammar = JoinRule([PREFIX, sequence("b", min_length=4), SUFFIX])
        check_match_failure(
            bad_string=PREFIX + bad_string + SUFFIX,
            good_bytes=PREFIX.encode() + good_bytes,
            failure_byte=failure_byte,
    ...
    def test_bad_repeats_max_length(
        ...
        PREFIX = "AAA"
        SUFFIX = "BBB"
        grammar = JoinRule([PREFIX, sequence("b", max_length=2), SUFFIX])
        check_match_failure(
            bad_string=PREFIX + bad_string + SUFFIX,
            good_bytes=PREFIX.encode() + good_bytes,
            failure_byte=failure_byte,
    ...
    def test_bad_repeats_min_max_length(
        ...
        PREFIX = "AAA"
        SUFFIX = "BBB"
        grammar = JoinRule([PREFIX, sequence("b", min_length=1, max_length=2), SUFFIX])
        check_match_failure(
            bad_string=PREFIX + bad_string + SUFFIX,
            good_bytes=PREFIX.encode() + good_bytes,
            failure_byte=failure_byte,
