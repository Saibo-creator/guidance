...
from .._grammar import select, GenRegexTerminalRule, quote_regex, capture, token_limit, with_temperature
...
def gen(
    ...
    *,
    ...
    if tools is not None:
        ...
        options = [
            GenRegexTerminalRule(body_regex=regex, stop_regex=gen_stop, save_stop_text=save_stop_text, max_tokens=max_tokens)
        ]
        for i, tool in enumerate(tools):
            ...
            options.append(
                capture(
                    GenRegexTerminalRule(body_regex=regex, stop_regex=quote_regex(tool_call_prefix), max_tokens=max_tokens),
                    name=f"tool{i}"
                )
            )
    ...
    pattern = GenRegexTerminalRule(body_regex=regex, stop_regex=gen_stop, save_stop_text=save_stop_text, name=tagged_name, max_tokens=max_tokens)
